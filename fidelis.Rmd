...
fidelis_colours <-
  c("#FBB03B", # Pastel orange
    ...
    "#3C85FA", # Bright blue
    "#8ABAE7", # Cyan
    "#D8E5F3", # Light grayish blue.
    "#6e5200", # Very dark orange
    "#e4fa3c" # Bright yellow.
    )
bar_chart_colours <- c("#D8E5F3", "#8ABAE7", "#3C85FA", "#001B6D", "#FBB03B")
```
...
# Works only with the Excel version of the data set.
fidelis <- read_excel("~/FIDELIS_LandscapeSurvey_Data.xlsx")
```
...
characteristics_levels |>
  ...
  scale_color_manual(values = bar_chart_colours) +
  scale_fill_manual(values = bar_chart_colours) +
...
characteristics_certification |>
  ggplot(aes(x = n, y = Status)) +
  geom_bar(stat='identity', fill = "#FBB03B") +
...
**Does your repository use persistent identifiers (PIDs) for digital objects or metadata elements?**  
...
training_find <-
  fidelis |>
  select(
    "Where do you currently (or have you previously) find/receive training on topics related to Trustworthy Digital Repositories? Select all applicable."
  ) |>
  rename("Source" = 1) |>
  drop_na()
...
training_find <-
  ...
  mutate(
    Source = recode(
      Source,
      "In specific communities I am part of (e.g. CESSDA, ELIXIR, RDA)" = "Part of specific communities",
      "From peers – (inter)nationally" = "Peers",
      "European project-based training initiatives (e.g. EOSC-Nordic, FAIRsFAIR, FAIR-IMPACT)" = "European projects",
      "At my host institution – from my peers" =  "Host institution (peers)",
      "Offerings available online" = "Online", "National training initiatives" = "National initatives",
      "International training initiatives (e.g. COAR)" = "International initatiotives",
      "At my host institution – formally offered training" = "Host institution",
      "Nowhere / I am not aware of any training opportunities related to Trustworthy Digital Repositories" = "Nowhere/Not aware"
    )
  )
...
objective <- fidelis |>
  ...
  rename("Indicate" = 1) |>
  count(Indicate) |>
  mutate("Statement" = "Achieve objective/outcome")
...
The FIDELIS network will be built within the FIDELIS project. This network of Trustworthy Digital Repositories (TDRs) will support the development and growth of TDRs within the EOSC ecosystem; foster harmonisation and interoperability across repositories to enable an EOSC federation of TDRs; and strengthen the upskilling of repositories and expand the network through an active training and support programme. These final questions cover your initial thoughts and expectations on the network so it can be shaped to support the needs of the community.
...
# Other responses
kable(fidelis |>
        select("Other, namely:...190") |>
        rename("Other mechninisms" = "Other, namely:...190") |>
        drop_na())
```
...
# Other responses
kable(fidelis |>
        select("Other, namely:...193") |>
        rename("Other mechninisms" = "Other, namely:...193") |>
        drop_na())
```
...
# Visualisations
**Training topics**
```{r training topics visualisations, echo=FALSE}
#**Which of these topics have you previously received training or support on?**
a <-
  training_topics_recived |>
  rename(Received = n)
...
#bind
c <- merge(a, b, by = "Topic")

c <-
  c |>
  mutate(Gap = Interested - Received) |>
  mutate(
    Topic = recode(
      Topic,
      "Developing and sustaining the repository service" = "Develop/sustain repository",
      "Stakeholder engagement and communication" = "Engagement/communication",
      "FAIR-enabling qualities (how to make and keep digital objects in your repository FAIR)" = "FAIR-enabling",
      "Technical infrastructure and repository software" =  "Infrastructure and software"
    )
  )
...
d <-
  a |>
  rename(n = "Received") |>
  mutate(Training = "Received")
e <- b |>
  rename(n = "Interested") |>
  mutate(Training = "Interested")
...
# group bar chart
ggplot(f, aes(fill = Training, y = Topic, x = n)) +
...
rm(a, b, c, e, f)
```
```{r training formats visualisations, echo=FALSE}
#**Which format(s) of training or support have you previously experienced?**
a <-
  training_format |>
  rename(Received = n)
...
c <-
  a |>
  rename(n = "Received") |>
  mutate(Interest = "Received")
d <- b |>
  rename(n = "Interested") |>
  mutate(Interest = "Interested")
...
f <-
  ...
  mutate(
    Format = recode(
      Format,
      "Seminars/webinars (i.e. listening to information being provided)" = "Seminars/Webinars",
      "Peer support (i.e. from direct colleagues or other repositories)" = "Peer support",
      "(online) workshops (i.e. some level of interactivity in a seminar/webinar)" = "Interactive workshops",
      "Dedicated time for independent work on a topic" = "Dedicated time",
      "Hands-on working sessions (e.g. hackathons)" = "Hands on",
      "Open hours (i.e. possibility to drop in and ask any questions you have)" = "Open hours"
    )
  )

# group bar chart
ggplot(f, aes(fill = Interest, y = Format, x = n)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  xlab("Responses") +
  ylab("") +
  scale_fill_manual(values = fidelis_colours) +
...
rm(a, b, c, e, f)
```
```{r characteristics levels by repository type, echo = FALSE}
as_deposited_type <- fidelis |>
  select("What levels of retention, curation, and preservation does your repository provide? For each level, please indicate the status that most accurately fits the situation of your repository. : As deposited - Digital object content and supporting metadata are distributed to users exactly as they are provided by depositors.",
         "Is your repository domain-specific or generalist?") |>
  rename("Provision" = 1,
         "Type" = 2) |>
...
deposit_compliance_type <- fidelis |>
  select("What levels of retention, curation, and preservation does your repository provide? For each level, please indicate the status that most accurately fits the situation of your repository. : Deposit compliance - Digital object content and supporting metadata deposited are checked for compliance with defined criteria, e.g. digital object formats, metadata elements, and compliance with legal and ethical norms.",
         "Is your repository domain-specific or generalist?") |>
  rename("Provision" = 1,
         "Type" = 2) |>
...
active_preservation_type <- fidelis |>
  select("What levels of retention, curation, and preservation does your repository provide? For each level, please indicate the status that most accurately fits the situation of your repository. : Active preservation - In addition to Deposit Compliance and Initial Curation, your repository takes long-term responsibility for ensuring that the digital objects and metadata can be understood and rendered as required by the designated community for reuse.",
         "Is your repository domain-specific or generalist?") |>
  rename("Provision" = 1,
         "Type" = 2) |>
...
characteristics_levels_type <-
  rbind(as_deposited_type,
        deposit_compliance_type,
        inital_curation_type,
        active_preservation_type)

rm(as_deposited_type,
   deposit_compliance_type,
   inital_curation_type,
   active_preservation_type)

# Set factors
characteristics_levels_type$Level <-
  factor(
    characteristics_levels_type$Level,
    levels = c(
      "Active preservation",
...
characteristics_levels_type$Provision <- 
  factor(
    characteristics_levels_type$Provision,
    levels = c(
      "No",
